...
protected:
    ...
    std::string test_ecu_uid = "TBOX-DEVICE-001";
    std::string test_key_id = "key-001";
};

TEST_F(KeyEngineTest, GenerateDeviceKey) {
    KeyPair key_pair;
    EXPECT_EQ(engine->generate_device_key(test_ecu_uid, test_key_id, key_pair), ErrorCode::SUCCESS);
    EXPECT_FALSE(key_pair.public_key.empty());
    EXPECT_TRUE(key_pair.private_key_exists);
    EXPECT_EQ(key_pair.key_id, test_ecu_uid + "+" + test_key_id);
}
...
TEST_F(KeyEngineTest, GenerateDuplicateKey) {
    ...
    KeyPair key_pair2;
    EXPECT_EQ(engine->generate_device_key(test_ecu_uid, test_key_id, key_pair2), ErrorCode::SUCCESS);
...
TEST_F(KeyEngineTest, GetDeviceKey) {
    KeyPair key_pair;
    engine->generate_device_key(test_ecu_uid, test_key_id, key_pair);
    
    KeyPair retrieved_key;
    EXPECT_EQ(engine->get_device_key(test_ecu_uid, test_key_id, retrieved_key), ErrorCode::SUCCESS);
...
TEST_F(KeyEngineTest, SignData) {
    ...
    EXPECT_EQ(engine->sign(test_ecu_uid, test_key_id, data, signature), ErrorCode::SUCCESS);
    EXPECT_FALSE(signature.empty());
}

TEST_F(KeyEngineTest, KeyExists) {
    EXPECT_FALSE(engine->device_key_exists(test_ecu_uid, test_key_id));
    
    KeyPair key_pair;
    engine->generate_device_key(test_ecu_uid, test_key_id, key_pair);
    
    EXPECT_TRUE(engine->device_key_exists(test_ecu_uid, test_key_id));
}
...
TEST_F(KeyEngineTest, DeleteDeviceKey) {
    ...
    EXPECT_EQ(engine->delete_device_key(test_ecu_uid, test_key_id), ErrorCode::SUCCESS);
    EXPECT_FALSE(engine->device_key_exists(test_ecu_uid, test_key_id));
}

TEST_F(KeyEngineTest, GetNonExistentKey) {
    KeyPair key_pair;
    EXPECT_EQ(engine->get_device_key(test_ecu_uid, test_key_id, key_pair), ErrorCode::KEY_NOT_FOUND);
}
...
TEST_F(KeyEngineTest, SignWithoutKey) {
    ...
    EXPECT_EQ(engine->sign(test_ecu_uid, test_key_id, data, signature), ErrorCode::KEY_NOT_FOUND);
}
