...
private:
    ...
    DiagResponse process_request_internal(DiagRequestType request_type,
                                         const std::vector<uint8_t>& request_data);  // unused in mock
};
...
ErrorCode DiagServiceAdapter::send_request(DiagRequestType request_type,
    ...
    std::lock_guard<std::mutex> lock(mutex_);
    threads_.emplace_back([this, request_type, request_data, callback]() {
...
DiagResponse DiagServiceAdapter::process_request_internal(DiagRequestType request_type,
    ...
    // Mock implementation: request_data is intentionally unused.
    // Returns fixed responses based on request type for testing purposes.
